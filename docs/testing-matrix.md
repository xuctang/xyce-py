...
| Examples | missing target audience labels, invalid notebook JSON, dirty executed outputs, non-compilable notebook code cells, beginner example drifting to low-level compiler seams | `tests/unit/test_examples.py` |
...
| Configurable Xyce features | exact directive/model/device/output emission, mapping construction, unknown config keys, unsafe paths, duplicate output declarations, workflow command shape, workflow failures, missing workflow outputs, XDM/ADMS defaults | `tests/unit/test_features.py`, `tests/unit/test_graph.py`, `tests/unit/test_api_contract.py`, `tools/release_smoke.py` |
...
| Solved graph projection | non-mutating graph copy, waveform row selection, node voltage annotation, ground/unmapped voltage skip, invalid row types, out-of-range rows, existing solved attribute collision | `tests/unit/test_models.py`, `tests/unit/test_models_contracts.py` |
